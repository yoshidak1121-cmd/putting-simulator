...
#### 運動方程式（実装：時間刻み Δt の離散化）

座標は **X（右方向）**, **Y（カップ方向）**。傾斜角 θ は「右傾斜を +」とします。

- 位置：**r = (x, y)**
- 速度：**v = (vx, vy)**、速さ：**|v| = sqrt(vx^2 + vy^2)**
- 転がりによる一定減速度：**aRoll > 0**（スティンプ値から算出）
- 傾斜による横方向加速度：**aSlope = g sinθ**

加速度を
- 傾斜：(**aSlope**, 0)
- 転がり抵抗：速度と逆向きに大きさ **aRoll**

として、連続系では

```
a = (g sinθ, 0) - aRoll * v / |v|    (|v| > 0)
```

成分表示：

```
ax = g sinθ - aRoll * vx / |v|
ay =        - aRoll * vy / |v|
```

これを時間刻み **Δt** で数値計算します（陽的オイラー法）。時刻 tn = nΔt において：

1) 速さ

```
|v|n = sqrt(vx_n^2 + vy_n^2)
```

2) 加速度（|v|n > 0 のとき）

```
ax_n = g sinθ - aRoll * vx_n / |v|n
ay_n =        - aRoll * vy_n / |v|n
```
...
```
vx_{n+1} = vx_n + ax_n * Δt
vy_{n+1} = vy_n + ay_n * Δt
```
